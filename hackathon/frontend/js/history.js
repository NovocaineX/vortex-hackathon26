...
function getStorageKey() {
  if (!currentUser) return 'forensica_history_guest';
  return `forensica_history_${currentUser.uid || currentUser.email}`;
}
...
export function addToHistory(doc, results) {
  ...
  localStorage.setItem(getStorageKey(), JSON.stringify(items.slice(0, 50)));
}
